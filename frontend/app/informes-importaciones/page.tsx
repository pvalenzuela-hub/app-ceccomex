...
export default function InformesImportacionesPage() {
  ...
  return <main className="page dashboard-page">
    ...
    <section className="report-layout">
      ...
      <label className="tariff-search">Arancel por código o glosa<input value={partidaText} onChange={(event) => void searchPartidas(event.target.value)} placeholder="Ej. 8528 o motocicletas" />{partidas.length ? <div className="tariff-options">{partidas.map((partida) => <button key={partida.codigo} type="button" onClick={() => { setFilter('partidas', [...(filters.partidas ?? []), partida.codigo]); setPartidaText(''); setPartidas([]) }}><strong>{partida.codigo}</strong><span>{partida.glosa}</span></button>)}</div> : null}</label>
      {(filters.partidas ?? []).length ? <p className="selected-values">Aranceles: {filters.partidas.join(', ')}</p> : null}</section>
    </section>
